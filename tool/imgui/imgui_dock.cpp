...
struct DockContext {
    ...
    struct Dock {
        ...
        ImVec2 getMinSize() const {
            if (children[0] == nullptr) return {16, 16 + GetTextLineHeightWithSpacing()};

            auto s0 = children[0]->getMinSize();
            auto s1 = children[1]->getMinSize();
        ...
        void setParent(Dock *dock) {
            parent = dock;
            for (Dock *tmp = prev_tab; tmp != nullptr; tmp = tmp->prev_tab) tmp->parent = dock;
            for (Dock *tmp = next_tab; tmp != nullptr; tmp = tmp->next_tab) tmp->parent = dock;
        }
        ...
        Dock &getFirstTab() {
            Dock *tmp = this;
            while (tmp->prev_tab != nullptr) tmp = tmp->prev_tab;
            return *tmp;
        }

        void setActive() {
            active = true;
            for (Dock* tmp = prev_tab; tmp != nullptr; tmp = tmp->prev_tab) tmp->active = false;
            for (Dock* tmp = next_tab; tmp != nullptr; tmp = tmp->next_tab) tmp->active = false;
        }
...
void ImGui::RootDock(const ImVec2 &pos, const ImVec2 &size) {
...
bool ImGui::BeginDock(const char *label, bool *opened, ImGuiWindowFlags extra_flags, const ImVec2 &default_size) {
